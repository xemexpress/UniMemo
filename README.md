...
###### 以下是對於Memo內容<small>（承諾按金、代買清單、交收時地）</small>的簡述補充：
...
系統根據用戶的起點和終點，演算途經代買點及交收點的路線，並優先推送額外需時最少的路線及Memo。用戶可循以下方式讓系統執行路線演算：

| 方式     | 具體內容                                                  |
| -------- | -------------------------------------------------------- |
| 具向搜尋  | 以當前位置為起點、用戶輸入的目的地為終點、作「來回」<small>（皆可更改）</small>，演算相關路線。 |
| 非具向搜尋 | 以用戶事先設定的常到地點<small>（即「預設點」)</small>作為可能的起點或終點，演算相關路線。 |

系統默認以每秒1.25米的步速，把「額外路程需時不多於15分鐘<small>（可更改）</small>」的路線視作「較順便」的情況。另外，以下是較理想的情況，並將作特別標示：
